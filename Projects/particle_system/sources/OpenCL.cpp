...
void OpenCL::initSquare(bool firsttime)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, m_vbo[Index::Position]);
	...
	if (firsttime)
	{
		m_glBuffer[Index::Position] = cl::BufferGL(m_context, CL_MEM_READ_WRITE, m_vbo[Index::Position], &err);
		if (err)
			std::cout << "ERROR (" << oclErrorString(err) << "): kernel loading failed." << std::endl;
		m_clBuffer[Index::Position] = cl::Buffer(m_context, CL_MEM_READ_WRITE, m_particleCount * sizeof(Particle), NULL, &err);
	}
	err = m_queue.enqueueWriteBuffer(m_clBuffer[Index::Position], CL_TRUE, 0, m_particleCount * sizeof(Particle), p);
	if (err)
...
void OpenCL::initVelocity(bool firsttime)
{
	cl_int err = 0;
	...
	if (firsttime)
	{
		m_glBuffer[Index::Velocity] = cl::BufferGL(m_context, CL_MEM_READ_WRITE, m_vbo[Index::Velocity], &err);
		if (err)
			std::cout << "ERROR (" << oclErrorString(err) << "): kernel loading failed." << std::endl;
		m_clBuffer[Index::Velocity] = cl::Buffer(m_context, CL_MEM_READ_WRITE, m_particleCount * sizeof(Vector3), NULL, &err);
		if (err)
			std::cout << "ERROR (" << err << ")" << oclErrorString(err) << std::endl;
	}
	err = m_queue.enqueueWriteBuffer(m_clBuffer[Index::Velocity], CL_TRUE, 0, m_particleCount * sizeof(Vector3), v);
	if (err)
		std::cout << "ERROR -> " << oclErrorString(err) << std::endl;
	glUnmapBuffer(GL_ARRAY_BUFFER);
}
...
void OpenCL::loadData(void)
{
	...
	glGenBuffers(1, &m_vbo[Index::Position]);
	glBindBuffer(GL_ARRAY_BUFFER, m_vbo[Index::Position]);
	...
	if ((err = m_kernel.setArg(0, m_clBuffer[Index::Position])))
		std::cout << "ERROR (" << err << ") " << oclErrorString(err) << std::endl;
	if ((err = m_kernel.setArg(1, m_clBuffer[Index::Velocity])))
		std::cout << "ERROR (" << err << ") " << oclErrorString(err) << std::endl;
...
void OpenCL::update(float frametime)
{
	glFinish();
	...
	buffers.push_back(m_glBuffer[Index::Position]);
	err = m_queue.enqueueAcquireGLObjects(&buffers, NULL, NULL);
	if (err != CL_SUCCESS)
	...
	{
		err = m_kernel.setArg(3, (Mouse::getPosition().x - 0.5f) * 8.f);
		err = m_kernel.setArg(4, 1.f - (Mouse::getPosition().y - 0.5f ) * 8.f);
		if (err != CL_SUCCESS)
			std::cout << "ERROR kernel args" << std::endl;
	}
	else
	{
		err = m_kernel.setArg(3, 0.f);
		err = m_kernel.setArg(4, 0.f);
	}
	err = m_kernel.setArg(2, frametime);
	if (err != CL_SUCCESS)
		std::cout << "ERROR kernel args" << std::endl;

	err = m_queue.enqueueNDRangeKernel(m_kernel, cl::NullRange, cl::NDRange(m_particleCount), cl::NullRange, NULL, &event);
	if (err != CL_SUCCESS)
	...
	m_queue.enqueueCopyBuffer(m_clBuffer[Index::Position], m_glBuffer[Index::Position], 0, 0, m_particleCount * sizeof(Particle), NULL, NULL);
	err = m_queue.enqueueReleaseGLObjects(&buffers, NULL, NULL);
	if (err != CL_SUCCESS)
