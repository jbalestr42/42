...
/*   Updated: 2016/04/02 18:01:36 by jbalestr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			create_rooms(t_graph *graph, t_room *rooms)
{
	...
	graph->count = rooms->num + 1;
	graph->array = (t_adj_list*)malloc(graph->count * sizeof(t_adj_list));
	graph->room_name = (char**)malloc(sizeof(char*) * graph->count);
	if (!graph->room_name)
		return (0);
	i = 0;
	while (i < graph->count)
