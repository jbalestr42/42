...
void Bureaucrat::signForm(Form & p_form)
{
	if (mn_grade <= p_form.getGradeToSign())
		std::cout << *this << " signs form (" << p_form << ")." << std::endl;
	else
		std::cout << *this << " cannot sign form (" << p_form << ") because grade is too low." << std::endl;
	p_form.beSigned(*this);
}

void Bureaucrat::executeForm(Form const & p_form)
{
	try
	{
		p_form.execute(*this);
		std::cout << *this << " executes (" << p_form << ")" << std::endl;
	}
	catch(std::exception & e)
	{
		std::cout << *this << " fail to execute " << p_form << std::endl;
		std::cout << e.what() << std::endl;
		if (mn_grade > p_form.getGradeToExec())
			throw GradeTooLowException();
	}
}
...
{
	p_ostream << p_bureaucrat.getName() << ", bureaucrat grade " << p_bureaucrat.getGrade();
	return p_ostream;
}
