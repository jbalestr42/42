...
class Admin extends CI_Controller
{
	...
	public function admin_logas()
	{
		...
		{
			$this->load->model('user_model', 'u_manager');
			$user = $this->u_manager->read('*', array("id" => $this->input->get('id')))[0];
			if (!empty($user))
			{
				$this->session->set_userdata('admin_login', $this->session->userdata('login'));
				$this->session->set_userdata('admin_passwd', $this->session->userdata('passwd'));
				$this->session->set_userdata('log_as', "1");
				$this->session->set_userdata('login', $user->login);
				$this->session->set_userdata('passwd', $user->password);
			}
		}
		redirect('index');
	}
	...
	public function admin_logs()
	{
		...
		$data['logs'] = $this->l_manager->read('*');
		$this->layout->view('admin/admin_logs', $data);
	}
}
