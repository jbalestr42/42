...
function url_img_class($img, $class, $uri)
{
	echo '<a class="'.$class.'" href="'.site_url($uri).'"><img src="'.$img.'"></a>';
	return '';
}
...
function url_icon_get($text, $icon, array $get, $uri, $popup = '')
{
	$str = "";
	foreach ($get as $k => $v)
	{
	...
	if ($popup)
		echo '<a href="'.site_url($uri).'?'.$str.'"><i class="icon-'.$icon.'"></i>'.htmlentities($text).'<span>'.$popup.'</span></a>';
	else
